function [  ] = interactWithSize( fignum )
%interactWithSize - demonstrate how to link up button press (+ sz change!)
%
%      usage: [  ] = interactWithSize( [fignum] )
%         by: lpzds1
%       date: Oct 09, 2013
%        $Id$
...
switch lower(evnt.Key)
    ...
    case {'uparrow'}
        disp('uparrow - make 10%  bigger ')
        position = get(h,'Position');
        set(h,'position', [position([1 2]),  position([3 4]).*1.1] );
    case {'downarrow'}
        disp('uparrow - make 10%  smaller ')
        position = get(h,'Position');
        set(h,'position', [position([1 2]),  position([3 4]).*0.9] );
    case {'q','escape'}
        disp('quit')
